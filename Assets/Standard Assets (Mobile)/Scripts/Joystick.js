...
var maxDistanceFromCenter = 50.0;
		...
		for(var i : int = 0;i < count; i++)
		{
			...
			{	
				...
				{					
					...
					var distance = Vector2.Distance(Vector2(defaultRect.x, defaultRect.y), guiTouchPos);
					if (distance < maxDistanceFromCenter) {
						gui.pixelInset.x = guiTouchPos.x;
						gui.pixelInset.y = guiTouchPos.y;
					}
					// When finger is outside joystick draging area
					else {
					
						var Csqr = Mathf.Pow(maxDistanceFromCenter, 2);
						var E = guiTouchPos.x - defaultRect.x;
						var D = guiTouchPos.y - defaultRect.y;
						
						var shiftXSign = E < 0.0 ? -1 : 1;
						var shiftYSign = D < 0.0 ? -1 : 1;
						
						E = Mathf.Abs(E);
						D = Mathf.Abs(D);
						
						var t = E / D;
						var tsqr = Mathf.Pow(t, 2);
						
						var A = Mathf.Sqrt(Csqr / (tsqr + 1));
						var B = A * t;
						
						gui.pixelInset.x = defaultRect.x + B * shiftXSign;
						gui.pixelInset.y = defaultRect.y + A * shiftYSign;
					}
				}
